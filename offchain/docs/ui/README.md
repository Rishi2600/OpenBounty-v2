...
| `src/types/submission.ts`, `src/constants/submissions.ts`, `src/utils/submissions.ts` | Bounty entries (mock mode only, `SUBMISSIONS_PREVIEW`):<br>• `getSubmitBlock` (who may enter and why not)<br>• `validateSubmission`<br>• `findSubmission`, `votesPerTier`, `wonTiers`, `getVoteCandidates` (map on-chain votes, which point at wallets, onto entries)<br><br>See [../features/judging.md](../features/judging.md). |
| `src/utils/scorecard.ts`, `src/hooks/useScorecard.ts` | Judge scorecards:<br>• `CRITERIA` (Innovation, Execution, Impact, each 1–5)<br>• `scoreTotal` (out of 15, or `null` until all three are rated)<br>• `loadScorecard` and `saveScorecard`: `localStorage` under `openbounty:scorecard:<bounty>:<judge>`, and every access is guarded<br>• `useScorecard(bounty, judge)` returns `scorecard` and `saveScore` |
| `src/utils/judging.ts` | `canVoteOnTier` and `votableTiers` (the same checks as `vote_winner`: before the deadline, no winner yet, not voted yet), and `sortEntries` (newest, or by your score) |

...
impl Escrow {
    ...
    pub const LEN: usize = 
        ...
        32 +                          // organizer: Pubkey
        4 + (5 * 32) +                // judges: Vec<Pubkey> (assuming max 5 judges)
        1 +                           // threshold: u8
        4 + (4 * PrizeTier::LEN) +    // tiers: Vec<PrizeTier> (assuming max 4 tiers)
        8 +                           // deadline: i64
        1 +                           // bump: u8
        1;                            // vault_bump: u8
}
...
impl PrizeTier {
    ...
    pub const LEN: usize = 
        8 +        // amount: u64
        1 + 32 +   // winner: Option<Pubkey> (1 byte for Some/None + 32 for Pubkey)
        1;         // claimed: bool
}
...
mod tests {
    ...
    fn test_escrow_space_calculation() {
        // Verify our space calculation is correct
        let expected = 8 + 32 + (4 + 5*32) + 1 + (4 + 4*41) + 8 + 1 + 1;
